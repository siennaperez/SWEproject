...
app.get('/user', authenticateJWT, async (req, res) => {
  try {
    const user = await User.findById(req.userId); // Use the user ID set by the middleware
...
app.put('/user', authenticateJWT, async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.userId, { displayName, bio, profilePhoto }, { new: true });
    res.status(200).json(user);
  } catch (err) {
