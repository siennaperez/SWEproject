...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    async function fetchProfile() {
      try {
        const response = await fetch(`http://10.136.107.113:3000/profile/${userId}`);
  ...
  const saveProfile = async () => {
    try {
      const response = await fetch(`http://10.136.107.113:3000/profile`, {
        method: 'PUT',
      ...
      if (response.ok) {
        console.log('ID:', userId); // Debug log
