...
const LoginScreen = () => {
  ...
  const handleGoogleLogin = async () => {
    const googleAuthUrl = 'http://10.138.217.191:3000/auth/google';
  
    try {
      if (await InAppBrowser.isAvailable()) {
        await InAppBrowser.open(googleAuthUrl, {
        ...
        Linking.openURL(googleAuthUrl); // Fallback if InAppBrowser is unavailable
      }
    } catch (error) {
