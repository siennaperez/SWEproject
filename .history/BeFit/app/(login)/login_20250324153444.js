...
const LoginScreen = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    const authUrl = `http://10.138.217.191:3000/auth/google?redirect_uri=${encodeURIComponent(redirectUri)}`;
  
    try {
      const result = await AuthSession.startAsync({ authUrl });
      console.log(result);
      // Handle the result here (e.g., get the user info after successful login)
    } catch (error) {
